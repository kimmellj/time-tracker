...
func (cr *CRUDController) ReadMany(cx *goweb.Context) {
	...
	rows, res := checkedResult(db.Query(fmt.sprint"SELECT * FROM clients"))
...
func main() {
	...
	clientsController := new(CRUDController{"clients"})
	projectsController := new(CRUDController{"projects"})
